...
class Unet(object):
    _defaults = {
        "model_path"        : 'logs/Epoch44-Total_Loss0.0483-Val_Loss0.0874.pth',
    ...
    def generate(self):
        ...
        if self.num_classes <= 21:
            ...
            self.colors = [(0, 0, 0), (255, 255, 255), (0, 128, 0), (128, 128, 0), (0, 0, 128), (128, 0, 128), (0, 128, 128), 
                    (128, 0, 0), (64, 0, 0), (192, 0, 0), (64, 128, 0), (192, 128, 0), (64, 0, 128), (192, 0, 128), 
    ...
    def detect_image(self, image):
        ...
        with torch.no_grad():
            ...
            #   取出每一个像素点的种类
            #---------------------------------------------------#
            pr = F.softmax(pr.permute(1,2,0),dim = -1).cpu().numpy().argmax(axis=-1)  # pr min=0,max=1
            #--------------------------------------#
            #   将灰条部分截取掉
            #--------------------------------------#
            pr = pr[int((self.model_image_size[0]-nh)//2):int((self.model_image_size[0]-nh)//2+nh), int((self.model_image_size[1]-nw)//2):int((self.model_image_size[1]-nw)//2+nw)]  # # pr min=0,max=1
